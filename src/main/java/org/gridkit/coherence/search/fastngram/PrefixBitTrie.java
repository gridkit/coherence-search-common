...
public class PrefixBitTrie extends PrefixBitTrieAlgo {
	...
	public PrefixBitTrie(int addressBits, int dataBits, int pageCapacity) {
		...
		tokenMask = (-1) >>> (64 - addressBits - dataBits);
		valueMask = dataBits == 0 ? 0 : (-1) >>> (64 - dataBits);
...
//		assert fhValidate();
	...
	@Override
	protected int nodePositionsUsed(int page, int slot) {
	...
	protected int addSlot(int page, int slot, int f) {
		...
		if (free(hpage) > 0) {
			...
			int refCount = nodePositionsUsed(page, slot);
